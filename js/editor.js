    ...
    /*!
{
  ...
  var MockServer = 'http://' + location.hostname + ':1338';
  ...
  $('#confirm-upload').click(function() {
    ...
    var u_name = $('#api_name').val();
    var u_url  = $('#api_url').val();
    ...
    u_url = u_url.replace('\\','/');
    if(!u_url.startsWith('/')) u_url = '/' + u_url;

    if(u_url.startsWith('/mock')) {
      alert('api地址不能以/mock开头');
      return;
    }

    if(confirm('确认上传您的Mock模板吗？\n(若存在同路径API，将覆盖源文件)')){
      $.ajax({
        url: MockServer + '/mock/',
        type: 'POST',
        data: JSON.stringify({
        ...
        if(json.status == 'sucess') {
          $('#i_ApiName').html(u_name);
          $('#i_url').html('接口地址： <a href="' + MockServer+u_url + '">' + MockServer+u_url + '</a>')
          $('#noti').fadeIn();
        }
        else {
          alert('上传失败');
